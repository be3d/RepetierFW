...
//myline
float Printer::zProbeOffset = EEPROM::zProbeHeight();
...
void Printer::homeZAxis() // Cartesian homing
{
    ...
    if ((MIN_HARDWARE_ENDSTOP_Z && Z_MIN_PIN > -1 && Z_HOME_DIR == -1) || (MAX_HARDWARE_ENDSTOP_Z && Z_MAX_PIN > -1 && Z_HOME_DIR == 1))
    {
		...
		transformCartesianStepsToDeltaSteps(currentPositionSteps, currentNonlinearPositionSteps);
#endif
        // TODO X-AXIS and Y-AXIS must be at home  move in IF

          //myline move head to the middle of the bed
        PrintLine::moveRelativeDistanceInSteps(axisStepsPerMM[X_AXIS]*(X_MAX_LENGTH - X_MIN_POS)/2,axisStepsPerMM[Y_AXIS]*(Y_MAX_LENGTH - Y_MIN_POS)/2,0,0,homingFeedrate[X_AXIS],true,true);
        //myline run servo down 
        HAL::servoMicroseconds(0,2500,2000);
...
#endif
       //disabled move PrintLine::moveRelativeDistanceInSteps(0,0,zCorrection,0,homingFeedrate[Z_AXIS],true,false);
        //myline commented
        //currentPositionSteps[Z_AXIS] = ((Z_HOME_DIR == -1) ? zMinSteps : zMaxSteps - Printer::zBedOffset * axisStepsPerMM[Z_AXIS]);
        //myline
         PrintLine::moveRelativeDistanceInSteps(0,0, - Printer::zBedOffset * axisStepsPerMM[Z_AXIS],0,homingFeedrate[Z_AXIS],true,true);
