...
void Commands::processGCode(GCode *com) {
    ...
    switch(com->G) {
        ...
        case 29: { // G29 3 points, build average or distortion compensation
                ...
                //myline
                //PrintLine::moveRelativeDistanceInSteps(IGNORE_COORDINATE,IGNORE_COORDINATE,  20 * axisStepsPerMM[Z_AXIS],0,homingFeedrate[Z_AXIS],true,true);
                Printer::moveTo(IGNORE_COORDINATE,IGNORE_COORDINATE,20,IGNORE_COORDINATE, HOMING_FEEDRATE_Z);
        ...
        case 31:  // G31 display hall sensor output
            ...
            //myline přepsaná G32 na G29
        case 29: // G32 Auto-Bed leveling
		{
#if defined(Z_PROBE_MIN_TEMPERATURE) && Z_PROBE_MIN_TEMPERATURE && Z_PROBE_REQUIRES_HEATING
            float actTemp[NUM_EXTRUDER];
            for(int i = 0; i < NUM_EXTRUDER; i++)
...
//myline
    ...
    case 565: // M565 Set Z probe offsets from extruder 0 tip
      {

        if(com->hasZ()) HAL::eprSetFloat(EPR_Z_PROBE_HEIGHT,com->Z);

           
        Com::printF(Com::tDefaultZOffset,Z_PROBE_HEIGHT,2);
        Com::println();
        Com::printFLN(Com::tActualZOffset,EEPROM::zProbeHeight(),2);
        Com::println();
